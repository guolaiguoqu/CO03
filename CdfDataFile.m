classdef CdfDataFile < handle
    ...
    methods
        ...
        % get next event
        % fill next event with the data in datafile
            ...
            if obj.isOpen()
                while 1
                    ...
                    if s(1) ~= '#'
                        ...
                        disp(s); % s is obj.id without whitespace
                    end
                end
                % parse event header
                expected = 5;
                ...
                %input from string 
                % runNumber  eventNumber  vertex-x  vertex-y  numTracks
                %head= vector of successfully read values
                %n=number of successfully read values
                if n == expected
                    % read tracks
                    expected = 5 * head(5);
                    %head(5) is the number of tracks
                    %each track contains five numbers
                    %complete track parameters= CdfTrack properties array
                    %expected number of values from size [5,head(5)]
                    [pars, n] = fscanf(obj.id, '%g', [5, head(5)]);
                    %input from file
                    %float number, size=[5,head(5)]
                    % reads float values in size
                    %pars is track parameters
                    if n ~= expected
